 ...
 */
static int zephir_update_property_quick(zend_class_entry *ce, zval *object, const char *property_name, zend_uint property_length, zval *value, ulong key TSRMLS_DC){
 ...
 * Variables must be defined in the class definition. This function ignores magic methods or dynamic properties
 */
static int zephir_update_property(zend_class_entry *ce, zval *object, char *property_name, unsigned int property_length, zval *value TSRMLS_DC){
	return zephir_update_property_quick(ce, object, property_name, property_length, value, zend_inline_hash_func(property_name, property_length + 1) TSRMLS_CC);
}
	...
	while ((type = *buf++) != 0) {
		...
		switch (type) {
			...
			case BSON_OID: {
				...
				zephir_update_property(mongo_ce_Id, value, "$id", strlen("$id"), str TSRMLS_CC);
				zval_ptr_dtor(&str);
			...
			case BSON_STRING: {
				...
				if ((len - 1) == 0) {
					...
					if (!MonGlo(global_str)) {
						MAKE_STD_ZVAL(MonGlo(global_str));
						ZVAL_EMPTY_STRING(MonGlo(global_str));
					}

					value = MonGlo(global_str);
					Z_ADDREF_P(value);
			...
			case BSON_ARRAY: {
				...
				/*if (type == BSON_ARRAY && strlen(buf) == 3) {

					if (!MonGlo(global_array)) {
						MAKE_STD_ZVAL(MonGlo(global_array));
						array_init(MonGlo(global_array));
					}

					value = MonGlo(global_array);
					Z_ADDREF_P(value);
			...
			case BSON_BOOL: {
				...
				CHECK_BUFFER_LEN(BYTE_8);
				int bool_value_tmp = (*buf++);
				if (bool_value_tmp) {

					if (!MonGlo(global_true)) {
						MAKE_STD_ZVAL(MonGlo(global_true));
						ZVAL_BOOL(MonGlo(global_true), 1);
					}

					value = MonGlo(global_true);
					//fprintf(stderr, "True=%d\n", Z_REFCOUNT_P(value));
				} else {

					if (!MonGlo(global_false)) {
						MAKE_STD_ZVAL(MonGlo(global_false));
						ZVAL_BOOL(MonGlo(global_false), 0);
					}

					value = MonGlo(global_false);
			...
			case BSON_NULL: {
				...
				if (!MonGlo(global_null)) {
					MAKE_STD_ZVAL(MonGlo(global_null));
					ZVAL_NULL(MonGlo(global_null));
				}
				value = MonGlo(global_null);
				Z_ADDREF_P(value);
				break;
			}
			...
			case BSON_INT: {
				...
				long lvalue = MONGO_32(*((int*)buf));
				switch (lvalue) {

					case 0:

						if (!MonGlo(global_zero)) {
							MAKE_STD_ZVAL(MonGlo(global_zero));
							ZVAL_LONG(MonGlo(global_zero), 0);
						}

						value = MonGlo(global_zero);
						Z_ADDREF_P(value);
					...
					case 1:
						...
						if (!MonGlo(global_one)) {
							MAKE_STD_ZVAL(MonGlo(global_one));
							ZVAL_LONG(MonGlo(global_one), 1);
						}

						value = MonGlo(global_one);
						Z_ADDREF_P(value);
					...
					case 2:
						...
						if (!MonGlo(global_two)) {
							MAKE_STD_ZVAL(MonGlo(global_two));
							ZVAL_LONG(MonGlo(global_two), 2);
						}

						value = MonGlo(global_two);
						Z_ADDREF_P(value);
					...
					case 3:
						...
						if (!MonGlo(global_three)) {
							MAKE_STD_ZVAL(MonGlo(global_three));
							ZVAL_LONG(MonGlo(global_three), 3);
						}

						value = MonGlo(global_three);
						Z_ADDREF_P(value);
		...
		int length = strlen(name) + 1, found = 0;

		do {

			if (interned_names->length == length && !memcmp(name, interned_names->name, length)) {
				zend_symtable_update(result, interned_names->pointer, length, &value, sizeof(zval*), NULL);
				found = 1;
				break;
			}
			++interned_names;
		...
		if (!found) {
			zend_symtable_update(result, name, length, &value, sizeof(zval*), NULL);
		}
